...
function getFilteredData(module) {
  switch (module) {
    ...
    case 'attendance': {
      ...
      let data = window.ATTENDANCE || [];
      if (classVal !== 'all') {
        data = data.filter(r => r.class === parseInt(classVal));
      }
      if (sectionVal !== 'all') {
        data = data.filter(r => r.section === sectionVal);
      }
      if (dateVal) {
        data = data.filter(r => r.date === dateVal);
      }
      return data;
    }
    default:
  ...
  switch (module) {
    ...
    case 'attendance':
      ...
      rows = data.map((r, idx) => {
        ...
        return [idx+1, student ? student.name : 'Unknown', r.class, r.section, r.date, r.status];
      });
      break;
    default:
      return null;
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const attPdfBtn = document.getElementById('attendanceExportPdf');
  if (attPdfBtn) attPdfBtn.addEventListener('click', () => exportToPDF('attendance'));
  const attExcelBtn = document.getElementById('attendanceExportExcel');
  if (attExcelBtn) attExcelBtn.addEventListener('click', () => exportToExcel('attendance'));
  const attCsvBtn = document.getElementById('attendanceExportCsv');
  if (attCsvBtn) attCsvBtn.addEventListener('click', () => exportToCSV('attendance'));
});
