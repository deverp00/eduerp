...
window.PAYMENTS = [];
window.ACTIVITIES = [];
window.ATTENDANCE = []; // <-- NEW: for attendance module
...
async function loadAllData() {
  try {
    const [students, teachers, fees, salary, payments, activities, attendance] = await Promise.all([
      getAllData('students'),
      getAllData('teachers'),
      ...
      getAllData('attendance'), // <-- NEW: load attendance too
    ]);
    ...
    window.ATTENDANCE = attendance; // <-- NEW
  } catch (error) {
