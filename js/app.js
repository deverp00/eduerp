...
async function renderDashboard() {
  ...
  // Get attendance percentage for today (if attendance module is loaded)
  let attendancePercent = 0;
  if (window.getTodayAttendancePercent) {
    try {
      attendancePercent = await window.getTodayAttendancePercent();
    } catch (e) {
      console.warn('Could not fetch attendance:', e);
    }
  }
  ...
  if (statsGrid) {
    statsGrid.innerHTML = `
      ...
      <div class="stat-card"><span class="stat-label">Today's Attendance</span><span class="stat-value">${attendancePercent}%</span></div>
    `;
  }
